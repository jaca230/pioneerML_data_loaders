...
struct EventSplitterEventInputs : public BaseBatch {
  ...
  std::shared_ptr<arrow::Array> endpoint_preds;   // float32, length = total_groups * 6
  ...
  std::shared_ptr<arrow::Array> y_edge;           // float32, length = total_edges

  size_t num_graphs{0};
  size_t num_groups{0};
};

struct EventSplitterEventTargets : public BaseBatch {
  std::shared_ptr<arrow::Array> y_edge;           // float32, length = total_edges
  size_t num_graphs{0};
};
