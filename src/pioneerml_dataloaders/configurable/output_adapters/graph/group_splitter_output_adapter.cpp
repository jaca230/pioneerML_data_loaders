...
std::shared_ptr<arrow::Array> GroupSplitterOutputAdapter::BuildNodeListColumn(
  ...
  if (!list_builder.Finish(&out).ok()) {
    throw std::runtime_error("Failed to finalize splitter node prediction list column.");
  }
  return out;
}

std::shared_ptr<arrow::Table> GroupSplitterOutputAdapter::BuildGraphTable(
  ...
  std::vector<std::shared_ptr<arrow::Field>> fields = {
      ...
      arrow::field("time_group_id", arrow::int64()),
...
void GroupSplitterOutputAdapter::WriteParquet(
    ...
    const std::shared_ptr<arrow::Array>& graph_event_ids,
    const std::shared_ptr<arrow::Array>& graph_group_ids) const {
  auto table = BuildGraphTable(node_pred, node_ptr, graph_event_ids, graph_group_ids);
